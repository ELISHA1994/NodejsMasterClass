    ...
    if (phone) {
        ...
        handlers._tokens.verifyToken(token,phone,function(tokenIsValid){
            if (tokenIsValid) {
                ...
                callback(403,{'Error' : 'Missing required token in header, or token is in valid'});
            }
        });
...
handlers._tokens.post = function(data,callback){
    // Check that all require fields are filled
    var phone = typeof(data.payload.phone) == 'string' && data.payload.phone.trim().length == 10 ? data.payload.phone.trim()  : false;
    var password = typeof(data.payload.password) == 'string' && data.payload.password.trim().length > 0 ? data.payload.password.trim() : false;
    if (phone && password) {
        ...
        _data.read('tokens',id,function(err,tokenData){
            if (!err && tokenData) {
                ...
                if (tokenData.expires > Date.now()) {
                    ...
                    callback(400,{'Error' : 'The tokens has already expired, and cannot be extended'});
                }
            } else {
                callback(400,{'Error' : 'Specifield tokens does not exist'});
            }
        });
    } else {
            ...
            if (!err && data) {
                _data.delete('tokens',id,function(err){
                    if (!err) {
                        ...
                        callback(500,{'Error' : 'Could not delete the specifield tokens'});
                    }
                });
            } else {
                callback(400,{'Error' : 'Could not find the specifield tokens'});
            }
        });
    } else {
        callback(400,{'Error' : 'Missing required field'});
...
// Checks - post
// Require data : Protocol, url, methods, sucessCodes, timeoutSeconds
// Optional date : none
handlers._checks.post = function(data,callback){
    ...
    if (protocol && url && method && successCodes && timeoutSeconds) {
        // Get the token from the heaaders
        var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
        ...
        if (protocol || url || method || successCodes || timeoutSeconds) {
            ...
            _data.read('checks',id,function(err,checkData){
                if (!err && checkData) {
                    ...
                    handlers._tokens.verifyToken(token,checkData.userPhone,function(tokenIsValid){
                        if (tokenIsValid) {
                            // Update the check were necessary
                            if (protocol) {
